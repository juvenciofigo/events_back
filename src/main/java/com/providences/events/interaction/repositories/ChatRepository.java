...
public interface ChatRepository extends JpaRepository<ChatEntity, String> {
    ...
    @Query("""
            ...
            AND p.guest.id = :guestId
            """)
    List<ChatEntity> findGuestChats(@Param("eventId") String eventId, @Param("guestId") String guestId,
            @Param("type") ChatType type);
    ...
    @Query("""
            ...
            """)
    List<ChatEntity> findAllOrganizerChats(@Param("organizerId") String organizerId);

    @Query("""
            SELECT DISTINCT chat
            FROM ChatEntity chat
            JOIN chat.participants p
            WHERE p.organizer.id = :participantId
            """)
    Page<ChatEntity> findByParticipant(@Param("participantId") String participantId, Pageable pageable);
